...
function SignUpWindow({ isSignUpModel, openSignUpModel, openModal }) {
  ...
  const validateInputs = () => {
    ...
    if (
      !inputs.email.value ||
      !/^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/.test(inputs.email.value.trim())
    ) {
      setInputs((prevInputs) => ({
        ...prevInputs,
        email: {
    ...
    if (
      ...
      !/[a-zA-Z]/.test(inputs.password.value.trim()) ||
      !/\d/.test(inputs.password.value.trim())
    ) {
      setInputs((prevInputs) => ({
        ...prevInputs,
        password: {
          ...prevInputs.password,
          errorMessage:
            " كلمة المرور يجب ان تكون اطول من 6 و تحتوي على ارقام وحروف ",
        },
      }));
      isValid = false;
    }
    ...
    if (
      !inputs.re_password.value ||
      inputs.re_password.value.trim() !== inputs.password.value.trim()
    ) {
      setInputs((prevInputs) => ({
        ...prevInputs,
        re_password: {
    ...
    <>
      ...
      >
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader className="flex flex-col gap-1 py-9">
                انشاء حساب
              </ModalHeader>
              ...
              <ModalBody>
                ...
                <div className="flex py-2 px-1 justify-between">
                  ...
                  >
                    ...
                    <span className="text-[#005B41] text-xs">
                      {" "}
                      الشروط والأحكام{" "}
                    </span>
